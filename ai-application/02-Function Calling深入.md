...
### 1.1 什么是 Function Calling —— 一个协商机制，不是远程执行

Function Calling 的名称容易让人误解。它听起来像是"模型调用你的函数"，但实际上正好相反——**代码调用模型、模型返回调用意图、代码执行函数、代码把结果喂回模型**。模型本身不执行任何函数，它只负责决定"此时应该调用哪个函数、传什么参数"。

更准确的理解是：Function Calling 是一个**协商机制**。你用 Tool Schema 告诉模型你有哪些工具可用，模型在生成回复时判断当前对话是否需要使用工具，如果需要就输出一个结构化的工具调用请求。你的代码接收到这个请求后执行实际的函数，然后把结果返回给模型，模型再基于结果生成面向用户的自然语言回复。

这个机制的价值在于：它把 LLM 从"纯文本生成器"升级为"可以触发外部动作的决策引擎"。模型不需要知道如何查询数据库、如何发送邮件——它只需要知道"什么时候该调用什么函数、给什么参数"，而实际的执行由你的代码完成。

### 1.2 完整的交互时序

理解 Function Calling 的交互时序至关重要。以下是单次工具调用的完整"来回"：

```
第一轮：
  你的代码 → API:  发送 messages（包含 system + user）
                   附带 tools 定义（告诉模型有哪些工具可用）

  API → 模型:     模型读取 messages 和 tools 定义

  模型 → API:     模型判断"用户想知道北京天气，我需要调用 get_weather"
                   输出 token（此时没有文本 content）
                   输出 tool_call: {function: "get_weather", arguments: '{"city":"北京"}'}

  API → 你的代码: 返回 response，其中 choices[0].message.tool_calls 包含调用请求
                   此时 message.content 为 None —— 模型在说"我要调工具"，而不是"我在回答"

[你的代码执行阶段]:
  你的代码:       json.loads(tool_call.function.arguments) → {"city": "北京"}
                  执行 get_weather(city="北京") → {"weather": "晴，25°C"}
                  构建 tool role 消息，将结果封装

第二轮：
  你的代码 → API:  发送更新后的 messages（现在多了 assistant(tool_calls) 和 tool 消息）

  API → 模型:     模型读取 messages，看到 tool 消息中有天气结果

  模型 → API:     模型基于天气结果生成自然语言回复
                   输出: "北京今天晴朗，气温25°C，适合户外活动"

  API → 你的代码: 返回 response，choices[0].message.content 包含最终回复
```

这个时序的关键点：**(1) 模型只输出调用意图，不执行函数；(2) 代码执行函数后，必须把结果作为 tool 消息重新发回 API；(3) 模型基于 tool 消息的内容生成最终回复**。如果工具执行失败，tool 消息中应该包含错误信息，模型会基于错误信息决定下一步（重试、改参数、或告知用户）。

### 1.3 最小可运行示例

下面的代码实现了上述完整时序。注意几个关键位置：`tool_choice="auto"` 让模型自主决策是否调工具；`if response_message.tool_calls` 检查模型是否发出了调用请求；`messages.append(response_message)` 将 assistant 的 tool_calls 消息加入历史（这一步经常被遗漏）；`tool_call_id` 必须与模型返回的 id 精确匹配。
...
运行这段代码，观察两次调用的行为差异。第一个问题"北京今天天气怎么样？"触发了 tool_calls，你会看到 `[Tool Call]` 日志输出，然后模型基于工具结果生成了带温度的回复。第二个问题"你好，你能做什么？"没有触发工具调用，模型直接回答。这是 `tool_choice="auto"` 的核心行为——模型自主决定是否需要工具。

### 1.4 tool_call 的数据结构

理解每个数据结构的字段含义，是调试 Function Calling 问题的基础。下面将整个交互过程中涉及的数据结构展开说明。
...
messages 的顺序是严格的：**[1] system -> [2] user -> [3] assistant(with tool_calls) -> [4] tool(with tool_call_id) -> [5] assistant(final reply)**。如果顺序错了（比如 tool 消息出现在 assistant 消息之前），API 会直接报错。这个顺序反映了对话的自然逻辑：用户提问 -> 模型决定调工具 -> 工具返回结果 -> 模型基于结果作答。

---

## 2. Tool Schema 设计指南

### 2.1 description 的写法技巧 —— 模型选工具的"唯一说明书"

Tool Schema 中的 `description` 字段，是模型判断"什么时候该调用这个工具"的**唯一依据**。模型不会看你的函数源码，不会看你的变量名——它只看 Schema。

这引出了一个重要的设计思维：把 `description` 当成"给一个聪明但对你系统一无所知的实习生的操作手册"来写。你需要回答三个问题：(1) 这个工具做什么？(2) 在什么情况下应该用它？(3) 在什么情况下不应该用它（与其他工具的区别）？

差的 description（如 "发送邮件"）只回答了问题1。好的 description 三个问题都回答了，还包括触发场景示例和注意事项。
...
第二个测试查询"电子邮件是什么？"是对 description 质量的试金石。好的 description 不会触发工具调用——模型能区分"用户想了解电子邮件这个概念"和"用户想发送一封电子邮件"。差的 description 可能误触发，因为"电子邮件"四个字匹配到了工具名。

### 2.2 parameters 的完整设计 —— 类型、约束、嵌套

一个设计精良的 parameters 定义应该像 API 文档一样严谨。每个字段需要：明确类型（string/number/boolean/array/object）；枚举约束可选值（enum）；用 minimum/maximum 约束数值范围；用 description 解释每个字段的含义和作用。

下面的 create_order 工具展示了实际项目中的完整 Schema 设计，涵盖了数组嵌套、对象嵌套、枚举、布尔、数值约束、正则模式等多种 JSON Schema 特性。
...
关键设计细节：`additionalProperties: False` 这个看似微小的设置实际上非常重要。没有它，模型可能自行添加 Schema 中未定义的字段（如给 items 加一个 "discount" 字段），导致下游代码处理失败。`minItems: 1` 确保模型不会创建空订单。`pattern` 约束手机号格式，避免模型编造一个无效号码。

### 2.3 parameters 设计的最佳实践

下面是一个 Schema 质量检查工具。在团队中引入这类自动化检查，可以大幅减少由 Schema 设计缺陷导致的问题。
...
### 3.1 tool_choice 参数详解 —— 四种控制模式

`tool_choice` 参数决定了模型在面对 tools 定义时的行为自由度，它有四种取值，形成从"完全自主"到"完全强制"的控制谱系。

- **"auto"**：模型自主决定是否调工具、调哪个。适合大多数场景，模型会在必要时调用，闲聊时直接回答。这是最"自然"的模式。
- **"none"**：即使传了 tools 定义，模型也绝不能调工具。用于临时禁用工具（如敏感操作前的确认环节），或测试"不加 tools 时的基线表现"。
- **"required"**：模型必须调工具。适用于"没有工具就无法完成任务"的场景——比如系统被设计为"用户的所有请求都必须转成 API 调用"。但要注意：如果用户说了"你好"，模型会硬着头皮找一个工具调用（参数可能乱填）。
- **指定工具名**：强制调用特定工具。适用于"对话已经进行到某一步，确定要调用某个工具"的场景，如多步工作流的中间步骤。
...
观察输出：当用户说"你好"且 tool_choice="required" 时，模型被迫调了一个工具（可能是 get_time），参数可能为空或乱填。这说明"required"是有风险的——只有在确定用户意图需要工具时才用它。

### 3.2 多工具时的选择逻辑

当有多个工具可用时，模型基于以下因素决定调用哪个（或多个）：(1) 用户意图与工具描述的匹配度——这是最重要的；(2) 参数是否可以上下文中推断——如果工具 A 需要的参数在对话中都有，而工具 B 需要用户补充，模型会优先选 A；(3) 是否有明确的工具名称提及——用户说"查一下天气"直接匹配 `get_weather`。

多工具选择的一个常见陷阱：当两个工具的功能有重叠时（如 `search_files` 和 `search_content`），模型可能选错。解决方案是在 description 中明确区分："本函数搜索文件名，不搜索文件内容"。
...
多工具系统中，如果两个工具描述有重叠，模型容易选错。最佳解决方案是把**工具名本身当作 description 的一部分**——让名字就说明功能边界。
...
当用户的问题需要多个工具配合时，有两种调用模式：**并行调用**和**串行调用**。

并行调用的前提是：两个工具的调用**完全独立**——A 的结果不影响 B 的参数，B 的结果也不影响 A。例如"北京和上海的天气分别是多少？"——两次 get_weather 调用互不依赖，可以一次请求同时发出。模型会在单次响应中返回多个 tool_calls，你的代码执行所有工具后一次性将结果发回。

串行调用的场景是：第二个工具的输入**依赖于**第一个工具的输出。例如"帮我查张三的订单"——你需要先调用 get_user_id(name="张三") 获取 UID，再调用 get_orders(user_id="U001") 查订单。这需要两轮 API 请求。

选择的标准很简单：**如果 B 的参数中有任何值需要从 A 的结果中获取 → 必须串行；否则可以并行**。并行调用的优势是减少轮次和延迟（两次 API 调用变一次），但它依赖于模型正确识别两个调用的独立性——如果模型判断错了（比如错误地并行调用了有依赖的工具），B 的参数会是编造的。
...
下面的代码演示了串行依赖场景的完整流程：第一轮获取 user_id，第二轮用 user_id 查订单，第三轮生成最终回答。关键点：在第二轮请求中，messages 已经包含了前一轮的 assistant(tool_calls) + tool 消息，模型可以从 tool 消息中读取 user_id 作为新一轮调用的参数。
...
下面的 `ToolExecutor` 类是一个生产级别的 Function Calling 执行框架。它封装了多轮工具调用循环（模型拿到工具结果后可能继续调工具）、并行调用处理、最大轮次限制（防止无限循环）和错误处理。这个类的核心价值在于：它让 Function Calling 从"手动管理 messages"变成了"传入 system + user，自动跑完整个工具链"。
...
### 6.1 工具执行失败的处理 —— 让模型帮你修正

Function Calling 的优雅之处在于：当工具执行失败时，你不需要在代码中处理每一种错误情况——你只需要把错误信息返回给模型，让模型自己决定下一步。这是把 LLM 当做"决策引擎"的典型应用。
...
关键观察：在测试1中，用户说"发邮件给张三"但没有提供邮箱地址，模型第一次调用时可能随便填了一个地址（如"zhangsan@example.com" 或直接 "张三"）。如果函数抛出了 ValueError（缺少 @），错误信息被返回给模型，模型看到"邮箱格式错误"后会重新尝试——比如询问用户正确的邮箱地址，或者尝试推断并加上 @domain。这就是"让模型帮你修正"的精髓。
...
Function Calling 中的工具可以分为三种基本模式，每种模式的**安全性要求**和**重试策略**截然不同。

**查询类（Query）**：只读操作，获取信息。特点：幂等（多次调用结果相同），无副作用，可以安全重试。如果查询失败，将错误信息返回给模型然后重试即可。典型例子：get_weather, search_docs, query_database。

**操作类（Action）**：写操作，改变外部状态。特点：有副作用（不可逆或代价高），需要谨慎重试。重复调用可能导致重复发送邮件、重复创建订单。因此操作类工具通常在调用前需要确认（系统询问用户"确定要...吗？"），或在 Schema 描述中提示模型"调用前请确认"。

**验证类（Validation）**：检查前置条件。特点：通常在其他操作前调用——先 validate_email 再 send_email，先 check_auth 再 query_database。验证失败则阻止后续操作。这类工具帮助实现"安全第一"的工具链，防止操作类工具在不安全的条件下被调用。

三种模式的组合构成了安全、健壮的工具链：**验证 -> 查询 -> 操作**。下面的代码演示了这一完整模式。
...
这是最容易犯的错误，API 错误信息也很明确。深层原因是：模型需要知道"哪个 tool 消息对应哪个 tool_call"，而 tool 消息通过 tool_call_id 关联到 assistant 消息中的 tool_calls 条目。如果 assistant 消息不在 messages 中，tool 消息就成了"孤儿"，模型无法理解它的上下文。
...
本章深入学习了 Function Calling 的完整体系。回顾关键理解：

- **调用链路与消息顺序**：定义 Schema（告诉模型有什么工具） -> 模型判断（输出 tool_calls） -> 代码执行函数 -> 返回结果（tool 消息） -> 模型再次生成回复。消息顺序必须严格：system -> user -> assistant(tool_calls) -> tool -> assistant(final)。这是整个交互协议的骨架，顺序错了 API 直接报错。

- **Schema 设计的核心**：description 是模型判断"何时调工具"的唯一依据，应包含功能、触发场景、注意事项三要素。parameters 的类型约束（enum、minimum/maximum、pattern）不仅让下游代码安全，也让模型生成的参数更准确。

- **tool_choice 的四种模式**："auto"（默认，大部分场景）、"none"（临时禁用工具）、"required"（强制使用但需谨慎）、指定工具名（多步工作流的中间步骤）。

- **并行 vs 串行**：独立工具并行（减少轮次）、依赖工具串行（分多轮请求）。判断标准：B 的参数是否需要 A 的结果。

- **错误处理的最佳实践**：不自己处理每种错误，把错误信息返回给模型，让模型这个"决策引擎"自己决定下一步（修正参数、询问用户、或放弃）。按错误类型区分重试策略（参数错误可重试、网络错误不重试）。

- **三种工具设计模式**：查询类（幂等，安全重试）、操作类（有副作用，需确认和谨慎重试）、验证类（前置条件，失败则阻止后续操作）。三者组合成"验证 -> 查询 -> 操作"的安全工具链。
