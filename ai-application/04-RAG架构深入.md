...
RAG（Retrieval-Augmented Generation，检索增强生成）是一种让 LLM 回答问题时**先查资料再回答**的技术范式。
...
LLM 的上下文窗口有限（GPT-4o 为 128K token，但实际使用中建议控制在 4K-8K 以内以保证质量）。一份 100 页的 PDF 可能有 10 万 token，必须切分成小块才能：

1. **检索精度**: 小块更容易精准匹配用户问题
2. **上下文窗口**: 小块可以在有限的窗口中放入更多候选
3. **成本**: 小块减少每次生成时的 token 消耗
...
### 3.4 chunk_size 和 chunk_overlap 调参指南
...
| 知识点 | 核心要点 |
|--------|----------|
| RAG 架构 | 离线索引(加载→切块→向量化→存储) + 在线查询(向量化→检索→排序→生成) |
| 文档加载 | PyMuPDF(PDF) / python-docx(Word) / BeautifulSoup(网页) |
| 文本切块 | 固定大小/递归字符，chunk_size 800-1200, overlap 100-200 |
| 稠密检索 | Embedding + 余弦相似度，语义匹配 |
| 稀疏检索 | BM25 算法，关键词精确匹配 |
| 混合检索 | alpha加权融合两路结果 |
| ReRank | Cross-Encoder 精排，粗检+精排两阶段 |
| 后处理 | 阈值过滤 + Jaccard 去重 + 上下文窗口分配 |
| 生成 | prompt 模板 + 来源标注 + fallback 策略 |
